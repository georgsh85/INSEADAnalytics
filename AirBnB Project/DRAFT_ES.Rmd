...
actual_1_predict_0 = -1e+05
actual_0_predict_1 = 0
actual_0_predict_0 = 2e+04
...
neighborhood               | (1=graduate school, 2=university, 3=high school, 4=others, 5=unknown, 6=unknown)
...
**Answers:**
* The following chart shows the box plots for LIMIT_BAL and PAY_AMT1 - PAY_AMT6 for class 1

```{r echo=FALSE, fig.height=4.5}
# Please ENTER the selected independent variables for which to draw box plots. 
# Please use numbers, not column names. E.g., c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8.
boxplots_independent_variables = c(2,13:18) # use only the PAY_ variables
...
* The variables that show the highest discriminatory power are PAY_0, PAY_2, PAY_3, and PAY_4 (see first two box plots) as well as LIMIT_BAL (see the two box plots above). The discriminatory power can be determined by large differences in the descriptive statistics that is shown in the box plots for the two classes of the dependent variable. 
...
* For very small *cp*, an extremely high number of *branches* is created, which can result in overfitting (i.e. the model works very well on the estimation data but not well on other sets of data) and inpracticality (basically runtime both in model generation as well as in prediction). The overfitting will reduce the trust we place in the classifier.
...
* For example, let's look at the US missile defense system: when the model predicts with a 99% probability threshold that the country is under attack, it sends a response strike, as this is a major event. A low threshold probability would be triggered more easily and lead to catastrophic events for which there would be no reason.
* For credit card default classification, we should use a threshold corresponding to the likelihood that appears in our model. Let's look at estimation_data: 

```{r echo=FALSE}
class_percentages=matrix(c(sum(estimation_data[,dependent_variable]==1)/nrow(estimation_data),sum(estimation_data[,dependent_variable]==0)/nrow(estimation_data)), nrow=1); colnames(class_percentages)<-c("Class 1", "Class 0")
rownames(class_percentages)<-"Percentages"
knitr::kable(class_percentages)
```

We can therefore choose 78% as a reasonable probability threshold.
...
<<<<<<< HEAD
* The actual "1" can be either predicted as a "0" or as a "1". The sum of these must therefore be 1. Similarly, the actual "0" can be either predicted as a "0" or as a "1" with a sum of 1.
* Having higher diagaonal values than off-diagonal values will serve to be accurate, as the prediction will have a higher probability of being true.


=======
* The **Actual 1** can be either predicted as a "0" or as a "1". The sum of the probability of these predictions must, therefore, be 100%. Similarly, the **Actual 0** can be either predicted as a "0" or as a "1" with a sum of probabilities of 100%.
* Having higher diagonal values than off-diagonal values will serve to be accurate, as the prediction will have a higher probability of being true (actual value equals predicted). In the above outcome, it can be seen that the prediction of **no default** is already fairly accurate, while predicting those that did **default** is still quite bad.
>>>>>>> 54be0af5b209b634ac4992ef6a9b766176d5c5ad
...
*
=======
* The 45-degree line corresponds to a **coin toss** with an outcome of 0.5. By selecting more validation cases (x-coordinate), exactly the same percentage of 1 class cases will be captured (y-coordinate). So the 45-degree line classifier does not help identify 1 class cases more efficiently. This can be also easily illustrated when calculating the **Area under the curve**, which will be the triangle between (0,0), (1,1), and (1,0).
>>>>>>> 54be0af5b209b634ac4992ef6a9b766176d5c5ad
...
*
=======
* To maximize profit, we would select the point which shows the highest estimated profit. For CART 1 and CART 2 this would be around 20%, for log reg it would be between 25% and 50% - depending on the randomly selected test data.
>>>>>>> 54be0af5b209b634ac4992ef6a9b766176d5c5ad
...
*
*
*The business decision to be made by the credit card issuer will likely be to maximize profit. Profits are maximized when the values in the diagonal of the confusion matrix are highest. By doing so, we would have monetary savings linked to the identification of potential defaults
=======
* Yes, the performance in the test data is comparable to the performance in the validation data as it can be seen in the hit ratio, confusion matrix, ROC curve, gains chart and profit curve. Since the test data is a random pick from the real data and a large enough set (3000 data points), we would expect a similar performance if deployed in practice. If not, we would need to reconsider the estimation data set and the variables that were taken into account for the models.
* Out of the three classifiers that we considered, we would recommend to use the Logistic Regression classifier because it outperforms CART1 and CART2 in all considered criteria. We would use 35% of the cases (or the corresponding probability threshold) because it maximizes the profit without compromising performance in the ROC and Gains curve. This would give a profit of around 7.5million as measured with the test data.
* The values in the profit matrix may change the relative performance of the classifiers (e.g., the shape of the ROC, gain curve and profit curve). However, logistic regression seems to outperform CART1 and CART2 in most of the cases. 
* The business decision to be made by the credit card issuer will likely be to maximize profit. The above classification models help decide to which applicants to provide credit, based on their key parameters and carefully defined threshold values. Profits are maximized when the values in the diagonal of the confusion matrix are highest. By doing so, we would have monetary savings linked to the identification of potential defaults.
>>>>>>> 54be0af5b209b634ac4992ef6a9b766176d5c5ad
