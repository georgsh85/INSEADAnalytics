...
AirBnB was founded in 2008 by *Brian Chesky*, *Joe Gebbia*, and *Nathan Blecharczyk* as *AirBed & Breakfast*, an online marketplace and hospitality service for short-term lodging. Over the past years, the share of professional hospitality providers has significantly increased and is now *crowding out* the private providers, threatening AirBnB's value proposition of *unique design and personal touch*. Within this context, the marketing department wants to run a campaign to attract more private providers. To do this, they requested the analytics department to create a tool that helps to attract potential landlords by helping them understand how much money they could earn with their respective apartment through AirBnb
...
ggplot(PricingData, aes(x=price)) + geom_histogram(binwidth=50) + ggtitle("Histogram of Prices")  + coord_cartesian(xlim = c(0, 1500))
```
...
```{r echo=FALSE, fig.height=4.5}
fit <- lm(scale(price)~room_type+reviews+neighborhood+overall_satisfaction+accommodates,data=PricingData.estimation)
