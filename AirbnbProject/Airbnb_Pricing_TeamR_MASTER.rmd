...
The *Mean Absolute Percentage Error* of a model gives us an idea about how well it can forecast, the lower being better.

```{r echo=FALSE, fig.height=4.5}
predicted.prices.testing<-predict(fit.step, PricingData.test) #predict the prices of the listings left for testing the model
...
predicted.prices.testing.log<-exp(predict(fit.log.step, PricingData.test))
...
```{r echo=FALSE, fig.height=4.5}
# repeat the same for the log model
predicted.prices.testing.int<-exp(predict(fit.int.step, PricingData.test))
percent.errors.int <- abs((PricingData.test$price-predicted.prices.testing.int)/PricingData.test$price)*100
```
The *Mean Absolute Percentage Error* for this model is `r round(mean(percent.errors.int),digits=2) `%.
...
As such, we tried to remove inconsistencies from our data at the start of the model, thus making sure that each of the parts of the data (estimation, validation, test) are consistent.
...
We tried to obtain the high levels of accuracy in our predictions - our Adjusted R score is converging towards the *60% level*. Our Mean Absolute Percentage Error is converging towards the vaunted level of 20% which is very encouraging in terms of the quality of our output.
