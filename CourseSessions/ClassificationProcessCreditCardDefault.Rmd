...
* The **Actual 1** can be either predicted as a "0" or as a "1". The sum of these must therefore be 100%. Similarly, the **Actual 0** can be either predicted as a "0" or as a "1" with a sum of 100%.
* Having higher diagaonal values than off-diagonal values will serve to be accurate, as the prediction will have a higher probability of being true. In the above outcome, it can be seen that the prediction of **no default** is already fairly accurate, while predicting those that did **default** is still quite bad.
...
* The 45-degree line corresponds to a **coin toss** with an outcome of 0.5. This can be easily shown when calculating the **Area under the curve**, which will be the triangle between (0,0), (1,1), and (1,0).
...
* To maximize profit, we would select the point which shows the highest estimated profit. For CART 1 and CART 2 this would be around 20%, for log reg it would be between 25% and 50%.
