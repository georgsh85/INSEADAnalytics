...
* For very small **cp**, an extremely high number of **branches** are created, which can result in overfitting (i.e. the model works very well on the estimation data but not well on other sets of data) and impracticality (basically high runtime both in model generation as well as in prediction). The overfitting will reduce the trust we place in the classifier.
...
* We would select the point closest to (0,1).
* The dotted 45-degree line corresponds to a random classifier. Since all three models (logistic regression, CART 1 and CART 2) are above the diagonal, their predictions are **better** than random. 
...
* Again, we should select the point closest to (0,1).
