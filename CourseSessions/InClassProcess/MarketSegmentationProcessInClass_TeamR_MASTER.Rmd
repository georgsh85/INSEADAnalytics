...
rmarkdown::render("CourseSessions/InClassProcess/MarketSegmentationProcessInClass_TeamR_MASTER.Rmd") 
...
* The highest correlation exists between the responses to questions 13 and 14. This is a perfect example of correlation since the questions are actually very similar. While #13 states that *When buying a boat, I tend to buy the latest and greatest*, #14 starts with *When buying accessories for my boat...*, otherwise identical. It is not surprising that many people respond similarly to those two questions. Other questions with high correlation include (16,17), (3,9), (6,7) and (27,28).
* In cases with very high correlation it might not add information to use both questions because of redundancy, instead one could reduce complexity by dropping one of them or calculating the average response.
...
* The table shows two "quality measures" used with PCA: **eigenvalue** and **percentage of variance explained**. The eigenvalue explains how much information each component captures. An eigenvalue > 1 indicates that the component captures more information than the average of any of the 29 original questions.The percentage of variance explained (and the cumulative percentage) describe how much of the original variance is contained in the components. The plot, **scree plot**, plots the eigenvalues in decreasing order which is the reason why the line will not increase.
* The typical criteria to select how many components shall be used are:
    + Eigenvalues should be greater than 1
    + The cumulative percentage of variance explained should be sufficiently large
    + All components up to the "elbow" of the scree plot (which can be difficult when the plot has multiple "elbows").
* We would like to see a few components that explain a very high percentage of variance. This way, we can reduce the complexity of the segmentation without loosing too much information. In our case, when looking at Eigenvalues above 1, we end up losing a lot of information (only capturing 52.74%). This makes us wonder if there are other factors to take into account or reconsider the quality of the data.
...
if (factor_selectionciterion == "variance")
  ...
  factors_selected = head(which(Variance_Explained_Table_copy[,"cumulative percentage of variance"]>= minimum_variance_explained),1) # corrected by Team R
...
To better visualize and interpret the factors we often "suppress" loadings with small values, e.g. with absolute values smaller than 0.44. In this case our factors look as follows after suppressing the small numbers:
...
* The first five factors indeed make business sense, with the following interpretations. Component 1 represents **passion for boating** with high loadings for Questions 27, 28, 20 (boating is true passion, number one thing in spare time, spending time on boat-related activities). Component 2 captures **trendiness** with high load on buying the latest and greatest boat and accessories. Component 3 is **adventurousness** with experiencing nature and escaping from everyday life. Component 4 is about **prestige**, considers the status, achievement and reward aspect of boating. Component 5 is **price-consciousness**, expressing preference for low price and no frills/ basic boat. The remaining 9 factors each capture one question from the survey.
* The number of factors to be chosen is already indicated above and might be based on statistical aspects such as having a significantly large eigenvalue or the factors together have sufficient explanatory power (e.g., 75% of variance captured). As for interpretation arguments, it is useful to choose factors in such a way that each attribute/question contributes only to one or a few factors (few high loadings per row) and that the attributes with high loadings differ significantly across the factors. Finally, the factors should capture relevant aspects of the business (problem) and not be too many. Namely, a business only can handle a certain level of complexity (cost benefit analysis) and, hence, a large number of factors should be avoided.
* We selected 14 factors, and started to outline the interpretation of the first 5 in the first bullet above. The interpretations of the factors are (from 1-14): passion, trendiness, adventurousness, prestige, price-consciousness, DIY-repair, shopping-around, brand-following, functionality-focus, expert-advised, lacking passion, socializing, premium-paying, sport-support. 
* The lesson that we learn is that we need to decide on a business solution to a technical problem: the factors that we are selecting should have a meaningful business interpretation, not only be justifiable by statistical arguments.
...
* The first individual (Obs1) seems to be driven by trends, prestige and adventure (factors 2,4,3) and care less about price, DIY-repairs and functionality (factors 5,6,9).
* The second individual (Obs2) values trends, prestige, expert opinion (factors 2,4,10), and cares less about shopping around, sports and price (factors 7,14,5).
* The third individual (Obs3) wants to have the latest-greatest and willing to pay a premium for it (factors 2,13), and bothers much less than average about functionality, DIY-repair, shopping around for deals and price (factors 9, 5-7).
* Obs10 shows a very different profile from the above three that values rather passion, adventure and good price for a basic boat (factors 3,5,1) and does not follow, research or pays premium for brands factors 7,8,13).

* For simplifying the analysis and ensuring easier interpretation of the factors used, we would select 14 of the initial variables which have the highest loading for each of the 14 factors. This way the segmentation variables are questions 27, 14, 19, 7, 2, 11, 1, 4, 8, 10, 29, 24, 5 and 26.
...
segmentation_attributes_used = c(28,15,20,8,3,12,2,5,9,11,30,25,6,27)
...
# Attention: kmeans has a stochastic initialization, which can result in slightly different outcomes everytime the code is executed
...
* These values compare the averages of the profiling variables of each segment relative to the average of the variables across the whole population. This helps us see differences between the segment averages and the general population and therefore confirm that we indeed found clusters.
* For the specific output above, we can conclude that the four clusters seem to be valid because on multiple profiling variables they significantly differ from the population average. Questions 1.2, 3, 5, 7, 9, 13, 14, 16, 17, 20, 26, 27 and 28, also 5, 17, 18 differentiate the four segments with ratios above 20% (in absolute value) for at least two clusters. These are around passion and knowledge for boating, power of the engine and brand consciousness. Nevertheless, Segments 1 and 4 seem to differ less from the population average than Segments 2 and 3.
    + Segment 1 is the "Boring Brand Buyers": they profile very close to the average population, valuing brand (Q1.4) and reputation of quality (Q5) to a slightly higher extent. They find low price (Q2), expertise (Q10), simplicity (Q12) and power (Q26) less important.
    + Segment 2 is the "Who Cares": they care much less than average about boating (clearly not a key passion, Q1.26-27), following the latest boat technology (Q13-17), and owning a boat clearly does not give them a sense of status or achievement (Q3,7,9).
    + Segment 3 is the "Boaters for Life": they are clearly into boating, which gives them a sense of passion (Q1.26-28), status (Q9), achievement (Q7). They are following the latest trends and know a lot about boats (Q13-17), including brands (Q3,7). They tend to have more powerful, longer boats (Q5-6), used for sports or socializing (Q9) alone or with friends and family (Q7). They would recommend a brand that they had positive experience with (Q16, 17) - although may buy a different brand next time (Q18). 
    + Segment 4 is the "Cheapos": they strongly prefer basic no frills boats (Q1.12), and value low price over brand (Q2) and powerful boats (Q12). They care less about brand (Q4) and reputation of quality (Q5) than the rest of the population.
* All segments seem to be able to afford boats having similar household income (Q12), with Segment 1 being more well off (maybe therefore they blindly follow brands and care less about value for money), and Segment 4 being less well off (therefore being price-conscious).
...
* We have identified three real out of four segments (as Segment 2 is "passive", less interested in buying boats): "Boring Brand Buyers", "Boaters for Life" and "Cheapos".
    a) The existence of these segments seems to be supported by statistics: there are numerous variables that significantly differentiate them from the population average. As mentioned above: Questions 1.2, 3, 5, 7, 9, 13, 14, 16, 17, 20, 26, 27 and 28, also 5, 17, 18 (around passion and knowledge for boating, power of the engine and brand consciousness) are 20% different from the population average for at least two clusters. Nevertheless, Segments 1 and 4 seem to differ less from the population average than Segments 2 and 3.
    b) Interpretation arguments: the segments can be interpreted well showing consistent characteristics and behaviors, such as those who show strong passion value brands more and low price less and vice versa. Detailed profiling is below. 
    c) Business arguments: we may drop Segment 2 ("Who cares") because it seems to be less interested in buying boats in general. The other three segments are active, and targetable - either with high brand value, strong features or good value for money options.
* The detailed analysis of the segments is in the answer of *Step 7*. As a recap of main characteristics:
Segment 1 is the "Boring Brand Buyers": they profile close to the average population but with higher income and without a great passion or interest for boating, therefore making their purchasing decision primarily based on brand and reputation (they can afford to simply follow brands and care less about value for money).
Segment 3 is the "Boaters for Life": they are very much into boating with a life-long passion, actively following the latest trends and brands, having powerful and longer boats for leisure activities, also enjoying the status and achievement of buying a boat.
Segment 4 is the "Cheapos": although they are more interested in owning boats than average, they strongly prefer basic no frills boats at a low price, being less well off than the average population.
* We iterated the analysis a few times, changing the number of components used (5-15, based on eigenvalues/ variance), the clustering method (hclust/kmeans) and also the number of segments (2-7). We found that the most consistently interpretable and significantly separable (from population average and each other) segments result from analysis with ~14 components that explain ~75% of the variance in the population.  1<eigenvalues suggested using 5 components but then we would have lost ~50% of the information. Finally, we had to iterate k-means itself to arrive at the most plausible segments.
* We advise CreeqBoat to target Segment 3 "Boaters for Life" with a modern boat with strong specifics and appealing design to set foot in the North American market. As for context, CreeqBoat aims to enter the North American market with targeted model(s) for prioritized customer segments. Expanding to this market is a necessity for the company to avoid becoming a victim of the boating industry crisis. We learnt from the cluster analysis that there are three relevant segments in this market: "Boring Brand Buyers", "Boaters for Life" and "Cheapos". Since CreeqBoat is new to this market, it potentially does not have a brand name yet that it can leverage to attract the "Boring Brand Buyers" segment. Furthermore, achieving high profitability from the "Cheapos" segment would require economies of scale, which CreeqBoat cannot again leverage as a new player on this market. Therefore, the company should target the "Boaters for Life" segment first, to establish brand value (i.e., potential for further future growth), with boat specifics that best cater to their needs. This means designing more powerful, longer boats in line with the latest boat technology, and in the marketing campaign emphasizing a life-long passion for boating as a high-quality leisure activity (sports and socializing) alone or with friends and family.
