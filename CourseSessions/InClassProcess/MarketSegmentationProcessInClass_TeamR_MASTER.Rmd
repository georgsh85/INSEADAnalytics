...
* In cases with very high correlation it might not be important to use both questions because of redundancy, instead one could reduce complexity by dropping one of them or calculating the average response.
...
* We iterated the analysis a few times, changing the number of components used (5-15, based on eigenvalues/ variance), the clustering method (hclust/kmeans) and also the number of segments (2-7). We found that the most consistently interpretable and significantly separable (from population average and each other) segments result from analysis with ~14 components that explain ~75% of the variance in the population.  1<eigenvalues suggested using 5 components but then we would have lost ~50% of the information.

* 1 and 7 - high content and low content. These take into account the Questions linked to a marked difference with the general population (in absolute value), and answering a set of cohesive questions. Our message in terms of strategy is to build 2 main products for the two key segments.
* Looking at the data, it seems as though we might need to iterate our survey and data analysis: some question are more actionable than others when looking at the difference between an identified cluster and the general population. Asking the questions in a different way, and modifying some questions to be more specific will help us see if other segments exist.
